    ...
    describe('Get /budget', () => {
        it('Should get all budgets', async () => {
            const res = await request(app)
                ...
                expect(res.body.budget.length).toBeGreaterThan(0)
        })
    })
    ...
    describe('Get /budget/:id', () => {
        it('Should get a single budget', async () => {
            const res = await request(app)
                ...
                .set("Cookie", `token=${token}`)
                expect(res.status).toBe(200)
                expect(res.body.getSingleBudget.budgetName).toBe('Shoe')
        })
    })
    ...
    describe('Update /budget/:id', () => {
        it('Should update a budget', async () => {
            const res = await request(app)
                ...
                // console.log(res.body)
                expect(res.status).toBe(200)
                expect(res.body.newBudget.budgetName).toBe('Bag')
        })
    })
    ...
    describe('Delete /budget/:id', () => {
        it('Should delete a budget', async () => {
            const res = await request(app)
                ...
                // console.log(res.body)
                expect(res.status).toBe(200)
                expect(res.body.message).toBe('Successfully deleted the budget')
        })
    })
})